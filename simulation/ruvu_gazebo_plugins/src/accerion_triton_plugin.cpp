...
void AccerionTritonPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  {
    ROS_FATAL_NAMED( "triton",  "triton plugin missing <bodyName>, cannot proceed");
    return;
  }
  else
  ...
  if (!this->link_)
  {
    ROS_FATAL_NAMED( "triton", "accerion_triton_plugin error: bodyName: %s does not exist\n",
  ...
  if (!_sdf->HasElement("loopRate"))
  {
    ROS_DEBUG_NAMED( "triton",  "triton plugin missing <loopRate>, defaults to 0.0"
             " (as fast as possible)");
    this->loop_rate_ = 0;
  }
  else
    this->loop_rate_ = _sdf->GetElement("loopRate")->Get<double>();
  ...
  if (this->reference_link_)
  {
    ROS_DEBUG_NAMED( "triton", "got body %s", this->reference_link_->GetName().c_str());
    this->frame_apos_ = 0;
    this->frame_aeul_ = 0;
#if GAZEBO_MAJOR_VERSION >= 8
...
void AccerionTritonPlugin::UpdateChild()
{
  ...
  {
      ROS_WARN_NAMED( "triton", "Negative update time difference detected.");
      this->last_loop_time_ = cur_time;
  }

  // loop rate control
  if (this->loop_rate_ > 0 &&
      (cur_time-this->last_loop_time_).Double() < (1.0/this->loop_rate_))
      return;
  ...
  double tmp_dt = cur_time.Double() - this->last_loop_time_.Double();
  this->last_loop_time_ = cur_time;
